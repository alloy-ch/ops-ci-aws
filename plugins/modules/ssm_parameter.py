...
#
# 2022-12-01 [Ringier AG]
# Originally ansible_collections/community/aws/plugins/modules/ssm_parameter.py
# Here we patch it to support creating/updating tags to parameter
...
options:
  ...
  tags:
    description:
      - A dictionary of tags to add to the parameter.
    required: false
    type: dict
seealso:
...
- name: Create or update key/value pair in AWS SSM parameter store with tier and tags
  community.aws.ssm_paramater:
    name: "Hello"
    description: "This is your first key"
    value: "World"
...
from ansible_collections.amazon.aws.plugins.module_utils.ec2 import AWSRetry, ansible_dict_to_boto3_tag_list, \
    boto3_tag_list_to_ansible_dict, compare_aws_tags
...
def update_tags(client, module):
    if module.check_mode:
        return

    try:
        param_name = module.params.get('name')
        orig_tags = client.list_tags_for_resource(
            ResourceType="Parameter",
            ResourceId=param_name)["TagList"]
        orig_tags_dict = boto3_tag_list_to_ansible_dict(orig_tags)
        new_tags_dict = module.params.get('tags')

        to_add, to_remove = compare_aws_tags(orig_tags_dict, new_tags_dict, purge_tags=True)

        if len(to_add) > 0:
            client.add_tags_to_resource(ResourceType="Parameter", ResourceId=param_name,
                                        Tags=ansible_dict_to_boto3_tag_list(to_add))
        if len(to_remove) > 0:
            client.remove_tags_from_resource(ResourceType="Parameter", ResourceId=param_name,
                                             TagKeys=to_remove)

    except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
        module.fail_json_aws(e, msg="tagging parameter")
...
def delete_parameter(client, module):
    ...
    try:
        ...
        botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:  # pylint: disable=duplicate-except
